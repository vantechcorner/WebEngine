...
<?php
	...
	if(isset($_POST['search_request'])) {
		try {
			...
			$search = $database->query_fetch("SELECT * FROM ".WEBENGINE_BAN_LOG." WHERE account_id ILIKE ? ORDER BY id DESC LIMIT 25", array($searchRequest));
			if(is_array($search)) {
				echo '<div class="row">';
				...
				echo '<table class="table table-striped table-condensed table-hover">';
					...
					foreach($search as $ban) {
						...
						echo '<tr>';
						echo '<td><a href="'.admincp_base("accountinfo&id=".$ban['account_id']).'">'.$ban['account_id'].'</a></td>';
						echo '<td>'.htmlspecialchars($ban['banned_by']).'</td>';
							echo '<td>'.$banType.'</td>';
